<template>
    <div :ref="(el: any) => {setCardRef(el)}" class="card-store-container flex-c" :data-id="node.id" :data-state="node.state" :style="storeCardStyle" @click="cardTapAction">
...
import { computed, VNodeRef, ref, CSSProperties } from 'vue';
...
const { node, nodeIndex, position } = props;

const storeCardStyle = computed(() => {
  return {
    'position': 'absolute',
    'zIndex': node.zIndex, 
    'top': `${position.top}px`, 
    'left': `${position.left + nodeIndex * position.width}px`
  } as CSSProperties
})
